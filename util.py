...
def perceptionLoss(pretrained_model="vgg16", device=device):
    ...
    if pretrained_model == "vgg16":
        m = tnn.Sequential(*list(torchvision.models.vgg16(pretrained=True).features)[:9]).to(device)
    elif pretrained_model == "resnet18":
        tmp_res = torchvision.models.resnet18(pretrained=True)
        m = tnn.Sequential(tmp_res.conv1, tmp_res.bn1, tmp_res.relu, tmp_res.maxpool, tmp_res.layer1).to(device)
        del tmp_res
    elif pretrained_model == "vgg19":
        m = tnn.Sequential(*list(torchvision.models.vgg19(pretrained=True).features)[:9]).to(device)
    else:
